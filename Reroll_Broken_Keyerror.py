...
def runonce(t,k,v):
    ...
    while sigma > 0:
        ...
        while (currentIntera<=desiredInter and triallimit!=0): #reroll for row with better intera until trial limit
        ...
        if(currentInter <= desiredInter):
            currentInter = currentIntera

            if(currentIntera > desiredInter):
                for colcom in colcombo:
                    valpair = []
                    for i in colcom:
                        valpair.append(tempRow[i])
